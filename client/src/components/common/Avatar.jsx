...
function Avatar({type,image,setImage}) {
  ...
  <div className='flex items-center justify-center'>
    ...
    { type==="sm" && ( <div className="relative  h-10 w-10"><Image src = {image} alt="avatar" className="rounded-full" fill/></div>
  )}
   { type==="lg" && ( <div className="relative  h-14 w-14"><Image src = {image} alt="avatar" className="rounded-full" fill/></div>
  )}
   { type==="xl" && (
    ...
    onMouseLeave={()=> setHover(false)}>
      ...
      >
        ...
        <span id="context-opener" onClick={ e => showContextMenu(e)}>Change <br/> DP</span>
        </div>
     <div className="flex items-center justify-center h-60 w-60"><Image src = {image} alt="avatar" className="rounded-full" fill/></div>
     </div>
  )}
  </div>
